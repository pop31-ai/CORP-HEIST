...
def run_tests():
    ...
    check("uid", c["user_id"] == 42)
    check("gold", c["gold"] == 99999)
    check("level", c["level"] == 77)
    check("net_worth", c["net_worth"] == 500000)
    check("has history", "history" in c)
    check("has stocks", "stocks" in c)
    check("has loot", "loot" in c)
    check("hero_levels len", len(c["hero_levels"]) == 12)
    check("passives len", len(c["passives"]) == 12)
    ...
    check("bytes len", len(data) == CHAR_SIZE)
    j = char_bytes_to_json(data)
    check("json uid", j["user_id"] == 7777)
    check("json gold", j["gold"] == 123456)
    check("json level", j["level"] == 88)
    check("json net_worth", j["net_worth"] == 2000000)
    check("json hero_levels", j["hero_levels"][:4] == [15, 12, 10, 8])

    # short data
    print("\n[Edge cases]")
    short = char_bytes_to_json(b'\x00' * 10)
    check("short data -> error", short.get("error") == "short data")

    # generate demo chars
    print("\n[Demo data]")
    generate_demo_chars(100)
    check("100 chars generated", len(CHARS_DB) == 100)
    uid = list(CHARS_DB.keys())[0]
    dc = CHARS_DB[uid]
    check("demo has stocks", len(dc["stocks"]) > 0)
    check("demo has loot", len(dc["loot"]) > 0)
    check("demo has history", len(dc["history"]) > 0)
    check("demo gold > 0", dc["gold"] > 0)
    check("demo net_worth > 0", dc["net_worth"] > 0)
    ...
    check("roundtrip uid", parsed["user_id"] == orig["user_id"])
    check("roundtrip gold", parsed["gold"] == orig["gold"])
    check("roundtrip level", parsed["level"] == orig["level"])
    check("roundtrip net_worth", parsed["net_worth"] == orig["net_worth"])
